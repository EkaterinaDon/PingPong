...
//  PingPong
//
//  Created by Ekaterina on 17.02.21.
//
...
- (void)animate {
    self.ball.center = CGPointMake(self.ball.center.x + self.dx*self.speed,
                                   self.ball.center.y + self.dy*self.speed);
...
- (BOOL)goal
{
    if (self.ball.center.y < 0 || self.ball.center.y >= SCREEN_HEIGHT) {
        ...
        if (gameOver) {
            [self displayMessage:[NSString stringWithFormat:@"Игрок %li выиграл",
                                  (long)gameOver]];
        } else {
            [self reset];
        }
